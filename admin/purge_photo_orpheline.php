...
while ($file = readdir($fp_dir)) {
	...
	// On teste sur la photo num�ro 1  
	if (ereg("^a([0-9]*)_1_photo.jpg", $file, $regs)) {
		...
		echo "Annonce num�ro $ida_tmp<br>";

		if (test_annonce_existe($ida_tmp)) {
...
echo "Il y a $i_supp photo � supprimer<br/>";

//-------------------------------------------------------------------------------------
// On teste si il y a une annonce pour cette photo
function test_annonce_existe($ida) {

	$query = "SELECT * FROM ano WHERE ida='$ida'";
	$result = dtb_query($query, __FILE__, __LINE__, DEBUG_PHOTO_ORPHELINE);
