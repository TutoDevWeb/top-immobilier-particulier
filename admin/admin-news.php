...
<body onload="init_count();">
	...
	echo "<p><a href='/admin/admin-news.php?action=start'>Retour à l'accueil de l'Administration</a></p>";

	check_arg($connexion, $action, $news_url, $news_titre, $news_description, $news_contenu, $news_connexe);

	if ($action == 'start') print_start($connexion);

	if ($action == 'creer_news') {
		creer_news($connexion, $news_url, $news_titre, $news_description, $news_contenu);
		print_start($connexion);
	}

	if ($action == 'choisir_news') {
		if (news_existe($connexion, $news_url)) {
			print_form_news($news_url);
			print_form_ajouter_news_connexe($connexion, $news_url);
			print_form_supprimer_news_connexe($connexion, $news_url);
	...
	if ($action == 'modifier_news') {
		if (news_existe($connexion, $news_url)) {
			modifier_news($connexion, $news_url, $news_titre, $news_description, $news_contenu);
	...
	if ($action == 'supprimer_news') {
		supprimer_news($connexion, $news_url);
		print_start($connexion);
	}
	if ($action == 'ajouter_news_connexe') {
		ajouter_news_connexe($connexion, $news_url, $news_connexe);
		print_start($connexion);
	}
	if ($action == 'supprimer_news_connexe') {
		supprimer_news_connexe($connexion, $news_url, $news_connexe);
		print_start($connexion);
	}

	if ($action == 'generer_le_flux') {
		generer_le_flux();
		print_start($connexion);
	}
	if ($action == 'refaire_les_dates') {
		refaire_les_dates($connexion);
		print_start($connexion);
	}
	...
	<?PHP
	//------------------------------------------------------------------------------------------------
	function print_start($connexion) {
		print_form_news($connexion);
		print_form_modifier_news($connexion);
		print_form_supprimer_news($connexion);
	...
	//------------------------------------------------------------------------------------------------
	function print_form_news($connexion, $news_url = "") {

		if (trim($news_url) != "") {
			$query  = "SELECT news_titre,news_description,news_contenu FROM news WHERE news_url='$news_url'";
			$result = dtb_query($connexion, $query, __FILE__, __LINE__, DEBUG_ADMIN_NEWS);
	...
	?>
		...
		<form action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method='post'>
			<fieldset>
				<?PHP
				if (trim($news_url) == "") echo "<legend>Cr�er</legend>";
				else echo "<legend>Modifier</legend>"; ?>
				...
				<p><label for='news_description'>R�sum� du contenu de la News (news_description)</label>
					...
					&nbsp;&nbsp;
					<input id='news_description_count' name='news_description_count' type='text' size="5" maxlength="3" readonly="readonly" />
				</p>
				<p>
					...
					<strong>1500 c'est une bonne taille</strong>
				</p>
				...
				?>
					<p><label for='news_url'>url de la News (news_url)</label>
						...
						&nbsp;&nbsp;
						<input id='news_url_count' name='news_url_count' type='text' size="5" maxlength="3" readonly="readonly" />
					</p>
					<p>
					<?PHP
				}
	...
	//------------------------------------------------------------------------------------------------
	function creer_news($connexion, $news_url, $news_titre, $news_description, $news_contenu) {
		...
		dtb_query($connexion, $insert, __FILE__, __LINE__, DEBUG_ADMIN_NEWS);
	}
	//------------------------------------------------------------------------------------------------
	function modifier_news($connexion, $news_url, $news_titre, $news_description, $news_contenu) {
		...
		dtb_query($connexion, $query, __FILE__, __LINE__, DEBUG_ADMIN_NEWS);
	}
	//------------------------------------------------------------------------------------------------
	function supprimer_news($connexion, $news_url) {
	...
	//------------------------------------------------------------------------------------------------
	function select_news($connexion) {
	...
	//------------------------------------------------------------------------------------------------
	function print_form_modifier_news($connexion) {
	?>
		<form action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method='post'>
			<fieldset>
				<legend>Modifier</legend>
				<?PHP select_news($connexion); ?>
	...
	//------------------------------------------------------------------------------------------------
	function print_form_supprimer_news($connexion) {
	?>
	...
	?>
		<form action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method='post'>
			<fieldset>
				<legend>Refaire les date du flux</legend>
	...
	//------------------------------------------------------------------------------------------------
	function news_existe($connexion, $news_url) {
	...
	function select_news_connexe_ajoutable($connexion, $news_url) {
		...
		$result = dtb_query($connexion, $query_ajoutable, __FILE__, __LINE__, 1);
	...
	//------------------------------------------------------------------------------------------------
	function print_form_ajouter_news_connexe($connexion, $news_url) {
	?>
		<form action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method='post'>
			<fieldset>
				<legend>Ajouter des news connexes</legend>
				<?PHP select_news_connexe_ajoutable($connexion, $news_url); ?>
	...
	//------------------------------------------------------------------------------------------------
	function ajouter_news_connexe($connexion, $news_url, $news_connexe) {
	...
	function select_news_connexe_supprimable($connexion, $news_url) {
	...
	//------------------------------------------------------------------------------------------------
	function print_form_supprimer_news_connexe($connexion, $news_url) {
	?>
		<form action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method='post'>
			<fieldset>
				<legend>Supprimer des news connexes</legend>
				<?PHP select_news_connexe_supprimable($connexion, $news_url); ?>
				<input type='hidden' name='news_url' value='<?PHP echo "$news_url"; ?>' />
	...
	//------------------------------------------------------------------------------------------------
	function supprimer_news_connexe($connexion, $news_url, $news_connexe) {
	...
	//------------------------------------------------------------------------------------------------
	function generer_le_flux($connexion) {
	...
	//------------------------------------------------------------------------------------------------
	function refaire_les_dates($connexion) {
