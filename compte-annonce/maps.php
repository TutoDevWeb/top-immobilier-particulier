...
if ($_REQUEST['action'] == 'enregistrer') enregistrer($connexion);
?>
...
<head>
	...
	<?PHP initialiser_geolocalisateur($connexion); ?>
...
<?PHP
function initialiser_geolocalisateur($connexion) {

	tracking_session_annonce($connexion, CODE_CTA, 'OK', "Entr�e dans Gerer les Cartes", __FILE__, __LINE__);
	...
	if (isset($_SESSION['maps_actif']) && $_SESSION['maps_actif'] == 1) {
		...
		tracking_session_annonce($connexion, CODE_CTA, 'OK', "Initialisation du geolocalisateur � l'adresse => $quart", __FILE__, __LINE__);
	} else {

		echo "var maps_actif = 0;\n";

		// Il faut lire les coordonn�es de la ville dans la database
		get_lat_lng($connexion, $_SESSION['zone_ville'], $_SESSION['zone_dept'], $ville_lat, $ville_lng);
		...
		tracking_session_annonce($connexion, CODE_CTA, 'OK', "Initialisation du geolocalisateur => Pas d'adresse connue", __FILE__, __LINE__);
	}

	echo  "</script>\n";
}
// Mattre les valeurs de la localisation en session puis aller sur la fiche
function enregistrer($connexion) {
	...
	if ($_REQUEST['maps_lat'] != '' && $_REQUEST['maps_lng'] != '') {
		...
		tracking_session_annonce($connexion, CODE_CTA, 'OK', "Enregistrement de la localisation : $quart", __FILE__, __LINE__);

		gotoo("/compte-annonce/fiche.php");
	}
}
//------------------------------------------------------------------------------------------------
function get_lat_lng($connexion, $ville, $dept, &$ville_lat, &$ville_lng) {


	$query = "SELECT v.ville_lat,v.ville_lng FROM loc_ville as v, loc_departement as d WHERE v.ville LIKE '$ville' AND v.idd = d.idd AND d.dept='$dept'";
