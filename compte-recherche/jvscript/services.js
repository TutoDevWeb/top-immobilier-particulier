...
function send_creer_alerte_baisse(tel_ins, prix) {
	...
	xhr.send('action=creer_alerte_baisse&tel_ins=' + encodeURI(tel_ins) + '&prix=' + encodeURI(prix));
...
function send_memoriser_financement() {
	...
	xhr.open("POST", "/compte-recherche/services-ajax.php", true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.send('action=memoriser_financement&fixe=' + encodeURI(fixe) + '&credit=' + encodeURI(credit) + '&apport=' + encodeURI(apport) + '&nb_annee=' + encodeURI(nb_annee) + '&taux_annuel=' + encodeURI(taux_annuel));
...
function send_creer_alerte_recherche() {
	...
	/*
	window.alert("Site en coours de test. Merci de votre compr�hension. Cliquer sur tous les OK");
	...
	*/
	...
	xhr.open("POST", "/compte-recherche/services-ajax.php", true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.send('action=creer_alerte_recherche&zone=' + encodeURI(zone) + '&zone_pays=' + encodeURI(zone_pays) + '&zone_dom=' + encodeURI(zone_dom) + '&zone_region=' + encodeURI(zone_region) + '&zone_dept=' + encodeURI(zone_dept) + '&zone_ville=' + encodeURI(zone_ville) + '&zone_ard=' + encodeURI(zone_ard) + '&dept_voisin=' + encodeURI(dept_voisin) + '&typp=' + typp + '&P1=' + P1 + '&P2=' + P2 + '&P3=' + P3 + '&P4=' + P4 + '&P5=' + P5 + '&sur_min=' + sur_min + '&prix_max=' + prix_max);
