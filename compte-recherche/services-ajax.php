...
if ($idc !== false) {
	...
	if ($action == 'memoriser_financement') {
		...
		if (memoriser_financement($connexion, $idc, $fixe, $credit, $apport, $taux_annuel, $nb_annee, __FILE__, __LINE__) === true) {
			tracking($connexion, CODE_CTR, 'OK', "Service Financement Mémorisation:$compte_email:$compte_pass", __FILE__, __LINE__);
			echo "ok_memoriser_financement";
		} else {
			tracking($connexion, CODE_CTR, 'KO', "Service Financement Mémorisation:$compte_email:$compte_pass:ko_memoriser_financement", __FILE__, __LINE__);
	...
	if ($action == 'creer_alerte_recherche') {
		...
		if (creer_alerte_recherche($connexion, $idc, $zone, $zone_pays, $zone_dom, $zone_region, $zone_dept, $zone_ville, $zone_ard, $dept_voisin, $typp, $P1, $P2, $P3, $P4, $P5, $sur_min, $prix_max, __FILE__, __LINE__) == true) {
			tracking($connexion, CODE_CTR, 'OK', "Service Alertes Recherche:$compte_email:$compte_pass:création", __FILE__, __LINE__);
			echo "ok_alerte_recherche";
		} else {
			tracking($connexion, CODE_CTR, 'KO', "Service Alertes Recherche:$compte_email:$compte_pass:ko_alerte_recherche", __FILE__, __LINE__);
	...
	if ($action == 'creer_alerte_baisse') {
		...
		if (creer_alerte_baisse($connexion, $idc, $tel_ins, $prix, __FILE__, __LINE__) === true) {
			tracking($connexion, CODE_CTR, 'OK', "Service Alertes Baisse:$compte_email:$compte_pass:création", __FILE__, __LINE__);
			echo "ok_alerte_baisse";
		} else {
			tracking($connexion, CODE_CTR, 'KO', "Service Alertes Baisse:$compte_email:$compte_pass:ko_alerte_baisse", __FILE__, __LINE__);
