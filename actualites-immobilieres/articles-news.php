  ...
  <div id='mainpan'>
    ...
    <div id='userpan'>
      ...
      make_ariane_news_articles($news_titre);
			print_news_by_url($news_url);
      print_news_date($news_url);
      print_news_connexe_by_url_order_dat($news_url);
			print_xiti_code('');
      ?>
    </div><!-- end userpan -->
  </div><!-- end mainpan -->
...
<?PHP
//------------------------------------------------------------------------------------------------------
function print_news_by_url($news_url) {
...
//------------------------------------------------------------------------------------------------------
function print_news_date($news_url) {
...
//------------------------------------------------------------------------------------------------------
function print_news_connexe_by_url($news_url) {
  ...
  if (  mysqli_num_rows($result) ) {
    ...
    while ( list($news_connexe) = mysqli_fetch_row($result) ) {
      $news = get_titre_news_by_url($news_connexe);
			$news_dat = get_dat_news_by_url($news_connexe);
...
//------------------------------------------------------------------------------------------------------
// Order by dat
function print_news_connexe_by_url_order_dat($news_url) {
...
//------------------------------------------------------------------------------------------------------
function get_data_news_by_url(&$news_titre,&$news_description,$news_url) {
...
//------------------------------------------------------------------------------------------------------
function get_titre_news_by_url($news_url) {
...
//------------------------------------------------------------------------------------------------------
function get_dat_news_by_url($news_url) {
  $query = "SELECT DATE_FORMAT(dat_creation,'%d-%m-%Y') FROM news WHERE news_url='$news_url'";
  $result = dtb_query($query,__FILE__,__LINE__,DEBUG_NEWS);
  if ( mysqli_num_rows($result) ) {
