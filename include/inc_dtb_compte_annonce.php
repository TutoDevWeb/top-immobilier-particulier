...
//--------------------------------------------------------------------------------------------------------
function insert_annonce($password, $file, $line) {

	$zone        = mysqli_real_escape_string($_SESSION['zone']);
	$zone_pays   = mysqli_real_escape_string($_SESSION['zone_pays']);
	$zone_region = mysqli_real_escape_string($_SESSION['zone_region']);
	$zone_dept   = mysqli_real_escape_string($_SESSION['zone_dept']);
	$zone_ville  = mysqli_real_escape_string($_SESSION['zone_ville']);
	$zone_ard    = mysqli_real_escape_string($_SESSION['zone_ard']);
	$zone_dom    = mysqli_real_escape_string($_SESSION['zone_dom']);
	$num_dept    = mysqli_real_escape_string($_SESSION['num_dept']);

	$tel_ins    = mysqli_real_escape_string($_SESSION['tel_ins']);
	$email      = mysqli_real_escape_string($_SESSION['sagmail']);
	$ok_email   = mysqli_real_escape_string($_SESSION['ok_email']);
	$tel_bis    = mysqli_real_escape_string($_SESSION['tel_bis']);
	$etat       = 'attente_paiement';


	$typp     = mysqli_real_escape_string($_SESSION['typp']);
	$nbpi     = mysqli_real_escape_string($_SESSION['nbpi']);
	$surf     = mysqli_real_escape_string($_SESSION['surf']);
	$prix     = mysqli_real_escape_string($_SESSION['prix']);
	$quart_s  = mysqli_real_escape_string($_SESSION['quart']);
	$blabla_s = mysqli_real_escape_string($_SESSION['blabla']);

	$maps_lat   = $_SESSION['maps_lat'];
	$maps_lng   = $_SESSION['maps_lng'];
	$maps_actif = $_SESSION['maps_actif'];

	$wwwblog    = mysqli_real_escape_string($_SESSION['wwwblog']);
	...
	$insert = "INSERT INTO ano (tel_ins,ip_ins,password,email,ok_email,
              ...
              VALUES ('$tel_ins','$ip_ins','$password','$email','$ok_email',
                      ...
                      '$maps_lat','$maps_lng','$maps_actif','$wwwblog')";
	dtb_query($insert, $file, $line, DEBUG_DTB_ANO);

	return (mysqli_insert_id());
}
//--------------------------------------------------------------------------------------------------------
// Mise � jour d'une annonce.
// On ne met � jour que la partie produit.
//--------------------------------------------------------------------------------------------------------
function update_annonce($etat, $file, $line) {
	...
	dtb_query($update, $file, $line, DEBUG_DTB_ANO);
}
//--------------------------------------------------------------------------------------------------------
function compte_annonce_existe($tel_ins, $file, $line) {
...
//--------------------------------------------------------------------------------------------------------
function get_zone($tel_ins, $file, $line) {
...
//--------------------------------------------------------------------------------------------------------
function get_etat($tel_ins, $file, $line) {
...
//--------------------------------------------------------------------------------------------------------
function get_ida($tel_ins, $file, $line) {
...
//--------------------------------------------------------------------------------------------------------
function get_email($tel_ins, $file, $line) {
...
//--------------------------------------------------------------------------------------------------------
function get_password($tel_ins, $file, $line) {
...
//--------------------------------------------------------------------------------------------------------
function get_annonceur_items($tel_ins, $file, $line) {
...
//------------------------------------------------------------------------------------------------
function get_dept_region_by_num_dept($num_dept, $dept, $region) {
...
//------------------------------------------------------------------------------------------------
function get_ip_dat($tel_ins, $ip_ins, $dat_ins) {
...
//--------------------------------------------------------------------------------------------------------
// Retourne les derni�res entr�e
function select_derniere() {
...
//--------------------------------------------------------------------------------------------------------
function set_date_modification($tel_ins, $file, $line) {

	$update = "UPDATE ano SET dat_mod=now() WHERE tel_ins='$tel_ins' LIMIT 1";
	dtb_query($update, $file, $line, DEBUG_DTB_ANO);
}
//--------------------------------------------------------------------------------------------------------
function get_date_modification($tel_ins, $file, $line) {
...
// Si simul vaut 1 on va g�n�rer un chiffre simul�
function get_nb_ano_by_typp_france($simul, &$nb_appartement, &$nb_maison, &$nb_loft, &$nb_chalet) {
	...
	if ($simul == true) {
		...
		// Les silver immo sont les annonces simul�es  
...
//--------------------------------------------------------------------------------------------------------
// Retourne le nombre d'annonce sur la r�gion
function get_nb_ano_by_typp_region($region, &$nb_appartement, &$nb_maison, &$nb_loft, &$nb_chalet) {
...
function get_nb_ano_by_typp_dept($zone_dept, &$nb_appartement, &$nb_maison, &$nb_loft, &$nb_chalet) {
...
function get_nb_ano_by_typp_ville($zone_ville, &$nb_appartement, &$nb_maison, &$nb_loft, &$nb_chalet) {
...
function get_nb_ano_by_typp_zone($zone, &$nb_appartement, &$nb_maison, &$nb_loft, &$nb_chalet) {
...
/*----------------------------------------------------------------------------------------------------*/
function demande_connexion_annonce($compte_tel_ins, $compte_pass, $code_refus, $file, $line) {

	// Si les identifiants sont l�, on s'en sert pour faire l'authentification
	if ($compte_tel_ins != '' && $compte_pass != '') {

		$compte_tel_ins_s = mysqli_real_escape_string($compte_tel_ins);
	...
	if (mysqli_num_rows($result)) {
      ...
      */
			tracking(CODE_CTA, 'OK', "CONNEXION USER: Compte Annonce : Connexion Accept�e<br/>$compte_tel_ins:$compte_pass:$ida", $file, $line);
		}
		return $ida;
