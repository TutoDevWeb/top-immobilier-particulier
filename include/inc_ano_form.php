...
?>
	...
	<form name="ano" method='post' action='<?PHP echo $_SERVER['PHP_SELF']; ?>' onsubmit="return valid_form_ano('<?PHP echo "$zone"; ?>');" <?PHP if (DEBUG_ANO_FORM) echo "onDblClick='fill_ano_form(this);'"; ?>>
		<table class='tab_ext'>
			...
			<tr>
				<td height='3'>
					<div align='right'><img src='/images/bord-bas.gif' alt='' /></div>
				</td>
			</tr>
		</table>
		<table class='tab_ext'>
			<tr>
				<td>
					<table class='tab_int'>
						<tr>
							<td class='label'>
								<p>Type<br /><?PHP print_select_typp($zone); ?></p>
							</td>
							<td width='3' style='background: url(/images/dotted-30C.gif) repeat-y'></td>
						...
						<tr>
							<td width='165'>
								<table width='100%' border='0' cellspacing='0' cellpadding='3'>
									<tr>
										<td class='label' align='center'>Votre t&eacute;l&eacute;phone 1<br /> <input <?PHP if (is_modif()) echo 'class=text_red'; ?> name='tel_ins' id='tel_ins' type='text' onkeyup="is_number('T�l�phone',this.value);" size='12' maxlength='10' <?PHP if (is_modif()) echo "readonly='readonly'"; ?> /></td>
									</tr>
									<tr>
									...
									<tr>
										<td align='center'>
											<p class='label'>Votre t&eacute;l&eacute;phone 2<br /><input name='tel_bis' id='tel_bis' type='text' size='12' maxlength='10' onkeyup="is_number('T�l�phone Optionnel',this.value);" /></p>
										</td>
									</tr>
									<tr>
		...
		<table class='tab_ext'>
			<tr>
				<td width='245'>
					<p>
						...
						<a href='noref/condition.php' target='_blank' rel='nofollow'>J'ai lu les conditions d'utilisation</a>
					</p>
				</td>
			...
			<tr>
				...
				<td colspan='2'>
					<p>&nbsp;</p>
					<p><em><strong>* <?PHP echo PRIX_ANNONCE, " Euros" ?></strong> &nbsp;pour <?PHP echo DUREE_ANNONCE ?> mois en ligne avec 5 photos</em> </p>
				</td>
			</tr>
		</table>
	</form>
...
?>
	<p>&nbsp;</p>
	<p>&nbsp;</p>
	<img src="/images/megaphone-deja-annonce.gif" alt="Il y a d�j� une annonce correspondant � ce num�ro" />
	<p>&nbsp;</p>
	<p>&nbsp;</p>
	<?PHP
	print_deconnexion();
...
?>
	...
	<select name="zone_dom" id="zone_dom">
		...
		<option value="Polynesie Fran�aise">Polynesie Fran�aise</option>
...
?>
  ...
  if ( zone_test.selectedIndex == 3 ) {
    ...
    surf.value = '100';
    prix.value = '800000';
    blabla.value = "Superbe villa avec vue sur la baie des Anges. Exposition plein Sud. Salon de 45 m� avec chemnin�e. Proximit� commerces, �coles, mer. 3 Chambres avec rangement. Salle de bains et une salle de douche. L'offre est � n�gocier";
  ...
  if ( zone_test.selectedIndex == 4 ) {
    ...
    surf.value = '220';
    prix.value = '340000';
    blabla.value = "Cause d�part. Villa F5, 220 m� avec sous-sol am�nag� en F3 de 85 m�. Villa F5 comprenant 3 chambres, spacieux, mezzanine 30 m�. F3, neuf, cuisine am�ricaine, 2 chambres, 2 salles de bains. Ensemble sur jardin paysager 1.015 m�. Vue panoramique. T�l�phoner de 7 h � 18 h m�tropole.";
  ...
  if ( zone_test.selectedIndex == 5 ) {
    ...
    surf.value = '350';
    prix.value = '1000000';
    blabla.value = "Un ryad � Marrakech id�al au coeur de la palmeraie. Pour vos vacances ou simplement un week end entre amis. La Palmeraie: A la sortie de la ville, sur la route de Casablanca, juste avant le pont sur l'oued Tensift, une �troite route permet de faire en voiture ou en cal�che un circuit de 22 km � travers la palmeraie. Peupl�e d'environ cent cinquante mille palmiers.";
