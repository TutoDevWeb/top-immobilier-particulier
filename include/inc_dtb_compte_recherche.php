<?PHP
/*----------------------------------------------------------------------------------------------------*/
function creation_compte_recherche($compte_email, &$compte_pass, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function compte_recherche_existe($compte_email, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function get_compte_arg_creation($compte_email, &$compte_pass, &$compte_ip, &$compte_date, &$compte_index_GMT, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function get_etat_compte_recherche($compte_email, $compte_pass, $file, $line) {

	$compte_email = mysqli_real_escape_string($compte_email);
	$compte_pass  = mysqli_real_escape_string($compte_pass);
...
/*----------------------------------------------------------------------------------------------------*/
function activer_compte_recherche($compte_email, $file, $line) {
...
// Renvoi faux si rien touver
function get_idc($compte_email, $compte_pass) {
	...
	if (isset($_COOKIE["connexion_compte_recherche"])) {
		...
		if ($sag_version == VERSION_SAG) {
			...
			$compte_email_s = mysqli_real_escape_string($compte_email);
			$compte_pass_s  = mysqli_real_escape_string($compte_pass);
			...
			tracking(CODE_CTR, 'OK', "Get idc:$compte_email:$compte_pass", __FILE__, __LINE__);
		} else return false;
	} else return false;
}
/*----------------------------------------------------------------------------------------------------*/
function get_email_compte_recherche($idc) {
...
/*----------------------------------------------------------------------------------------------------*/
function demande_connexion_recherche(&$compte_email, &$compte_pass, &$code_refus, $file, $line) {
	...
	if ($compte_email != '' && $compte_pass != '') {
		...
		tracking(CODE_CTR, 'OK', "Demande Connexion identifiant:$compte_email:$compte_pass", $file, $line);
		...
		tracking(CODE_CTR, 'OK', "Demande Connexion Cookie:$compte_email:$compte_pass", $file, $line);

		if ($sag_version == VERSION_SAG) {
			$compte_email_s = mysqli_real_escape_string($compte_email);
			$compte_pass_s  = mysqli_real_escape_string($compte_pass);
		} else {
			tracking(CODE_CTR, 'KO', "Echec Connexion Cookie:$compte_email:$compte_pass:$sag_version: ECHEC_VERSION", $file, $line);
			return COMPTE_RECHERCHE_CONNEXION_ECHEC_AUTHENTIFICATION;
		}
		// Dans ce cas il n'y a pas d'identifiants
	} else {

		tracking(CODE_CTR, 'KO', "Echec Connexion: pas identifiant : ECHEC_AUTHENTIFICATION", $file, $line);
	...
	if (mysqli_num_rows($result)) {
		...
		if (isset($_REQUEST['user']) && ($_REQUEST['user'] == 'adminsag')) {
			...
			tracking(CODE_CTR, 'OK', "CONNEXION ADMIN: Compte Recherche : Connexion Accept�e<br/>$compte_email:$compte_pass:$idc", $file, $line);
		} else {
			...
			// Mettre � jour la date de la derni�re connexion
			$query  = "UPDATE compte_recherche SET compte_date_connexion=now() WHERE idc='$idc' LIMIT 1";
			dtb_query($query, $file, $line, DEBUG_DTB_COMPTE);

			tracking(CODE_CTR, 'OK', "CONNEXION USER:  Compte Recherche : Connexion Accept�e<br/>$compte_email:$compte_pass:$idc", $file, $line);
		}
		...
		if (mysqli_num_rows($result)) {
			...
			if ($compte_bloquage == 'yes') {
				...
				tracking(CODE_CTR, 'KO', "Echec Connexion:$compte_email:$compte_pass: ECHEC_BLOQUAGE", $file, $line);
				return false;
			} else if ($compte_etat == 'inactif') {
				tracking(CODE_CTR, 'KO', "Echec Connexion:$compte_email:$compte_pass: ECHEC_INACTIF", $file, $line);
				$code_refus = COMPTE_RECHERCHE_CONNEXION_ECHEC_INACTIF;
				return false;
			}
		} else {
			if ($authentification_type == 'IDENTIFIANT') {
				tracking(CODE_CTR, 'KO', "Echec Connexion:$compte_email:$compte_pass: ECHEC_AUTHENTIFICATION_IDENTIFIANT", $file, $line);
				$code_refus = COMPTE_RECHERCHE_CONNEXION_ECHEC_AUTHENTIFICATION_IDENTIFIANT;
				return false;
			} else {
				tracking(CODE_CTR, 'KO', "Echec Connexion:$compte_email:$compte_pass: ECHEC_AUTHENTIFICATION", $file, $line);
				$code_refus = COMPTE_RECHERCHE_CONNEXION_ECHEC_AUTHENTIFICATION;
				return false;
			}
		}
...
/*----------------------------------------------------------------------------------------------------*/
function memoriser_financement($idc, $fixe, $credit, $apport, $taux_annuel, $nb_annee, $file, $line) {

	$idc = mysqli_real_escape_string($idc);
...
/*----------------------------------------------------------------------------------------------------*/
function get_financement($idc, &$finance_actif, &$fixe, &$credit, &$apport, &$taux_annuel, &$nb_annee, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function creer_alerte_recherche($idc, $zone, $zone_pays, $zone_dom, $zone_region, $zone_dept, $zone_ville, $zone_ard, $dept_voisin, $typp, $P1, $P2, $P3, $P4, $P5, $sur_min, $prix_max, $file, $line) {
	...
	$dept_voisin = mysqli_real_escape_string($dept_voisin);
...
/*----------------------------------------------------------------------------------------------------*/
function get_alerte_recherche($idc, $zone, $zone_pays, $zone_dom, $zone_region, $zone_dept, $zone_ville, $zone_ard, $dept_voisin, $typp, $P1, $P2, $P3, $P4, $P5, $sur_min, $prix_max, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function get_max_alerte_recherche($idc, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function supprimer_alerte_recherche($idar, $file, $line) {

	$idar = mysqli_real_escape_string($idar);
...
/*----------------------------------------------------------------------------------------------------*/
function creer_alerte_baisse($idc, $tel_ins, $prix, $file, $line) {

	$idc     = mysqli_real_escape_string($idc);
	$tel_ins = mysqli_real_escape_string($tel_ins);
	$prix    = mysqli_real_escape_string($prix);
...
/*----------------------------------------------------------------------------------------------------*/
function get_deja_alerte_baisse($idc, $tel_ins, $file, $line) {
...
/*----------------------------------------------------------------------------------------------------*/
function get_max_alerte_baisse($idc, $file, $line) {

	$idc     = mysqli_real_escape_string($idc);

	// Compter le nombre d'alerte � la baisse pour un utilisateur idc
	$query  = "SELECT idab FROM compte_recherche_alertes_baisse WHERE idc='$idc'";
	$result = dtb_query($query, $file, $line, DEBUG_DTB_COMPTE);
...
/*----------------------------------------------------------------------------------------------------*/
function supprimer_alerte_baisse($idab, $file, $line) {

	$idab = mysqli_real_escape_string($idab);
