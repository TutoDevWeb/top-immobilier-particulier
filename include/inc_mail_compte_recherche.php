<?PHP
//-----------------------------------------------------------------------------------------------
function mail_alerte_recherche($connexion, $compte_email, $list_alerte) {

	$url_site       = URL_SITE;
	$url_short_site = URL_SHORT_SITE;

	get_compte_arg_creation($connexion, $compte_email, $compte_pass, $compte_ip, $compte_date, $compte_index_GMT, __FILE__, __LINE__);
...
//-----------------------------------------------------------------------------------------------
function mail_alerte_baisse($connexion, $compte_email, $tel_ins) {
	...
	$message  = "Ce mail vous informe de la baisse du prix d'un produit sur lequel vous avez positionn� une alerte � la baisse.\r\n";
	$message .= "--------------------------------------------------------------\r\n";
...
//-----------------------------------------------------------------------------------------------
function mail_creation_compte_recherche($connexion, $compte_email) {
...
//-----------------------------------------------------------------------------------------------
function mail_relancer_activation_compte_recherche($connexion, $compte_email) {
...
//-----------------------------------------------------------------------------------------------
function mail_password_compte_recherche($connexion, $compte_email) {
...
function compte_send_mail($compte_email, $subject, $message, $trace = false) {
	...
	if (mail($compte_email, $subject, $mess, $headers)) {
		if ($trace == true) echo "<p class=text12cg>Envoi du mail � :$compte_email</p>\n";
		return true;
	} else {
		if ($trace == true) echo "<p class=text12cg>ECHEC envoi du mail � :$compte_email</p>\n";
		return false;
	}
}
