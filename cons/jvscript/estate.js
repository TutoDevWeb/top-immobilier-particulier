function Estate() {
	...
	//-----------------------------------------------------------------------------------
	// Mise � jour de l'ensemble des zones
	...
	//-----------------------------------------------------------------------------------
	// Mise � jour des param�tres qui concernent les coordonn�es
	...
	//-----------------------------------------------------------------------------------
	// Mise � jour des param�tres qui concernent le filtrage
	...
	//-----------------------------------------------------------------------------------
	// Mise � jour des param�tres qui concernent la zone
			...
			if (xhr.readyState == 4) {
				if (xhr.status == 200) {
					...
					if (estateList.length < 300) {
						...
						// On parcours la liste des annonces � afficher
	...
	function compute_stat(estateList) {
		...
		} else document.getElementById('stat_filtrage').innerHTML = 'Pas de s�lection';
	...
	function show_annonce(tel_ins, typp, nbpi, ville, ard, num_dept, quart, prix, surf, blabla) {
		...
		var str_piece = (nbpi == 1) ? " pi�ce" : " pi�ces";
	...
	function go_ville(zone_dept, zone_ville) {
		...
		if (zone_ville_selected != zone_ville)
			var info = "<div class='infoNav'><img src='/images/arrow5.gif' /><a href='#' class='navsearch11' onclick=\"switchzone.ville('" + encodeURI(zone_dept) + "','" + encodeURI(zone_ville) + "');\">Aller sur " + zone_ville + "</a></div>";
		else
			var info = "";
